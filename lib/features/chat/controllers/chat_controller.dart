  ...
  Future<void> pickOtherFile(bool isRemove, {int? index}) async {
    ...
    if(isRemove){
      ...
      if(platformFile != null) {
        ...
        if(_pickedFiles?.length == AppConstants.maxLimitOfTotalFileSent && validatedFiles != null &&   validatedFiles.length > AppConstants.maxLimitOfTotalFileSent){
          _pickedFIleCrossMaxLength = true;
        }
        if(_pickedFiles?.length == AppConstants.maxLimitOfTotalFileSent && validatedFiles != null && ImageValidationHelper.getMultipleFileSizeFromPlatformFiles(validatedFiles) > AppConstants.maxLimitOfFileSentINConversation){
